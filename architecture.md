...
├── evm-operators.el       # Операторы d/c/y с motions (основной механизм)
├── evm-registers.el       # VM Registers
├── evm-run.el             # Run Normal/Macro/Ex (fallback)
...
### Module 5: evm-operators
- Purpose: Операторы d/c/y с motions (основной способ редактирования)
- Dependencies: evm-core, evil
- Interface:
  - `evm-operator-delete ()` — запустить delete operator (ждёт motion)
  - `evm-operator-change ()` — запустить change operator
  - `evm-operator-yank ()` — запустить yank operator
  - `evm--parse-motion ()` — парсинг motion (w, e, b, iw, aw, i", a", etc.)
  - `evm--execute-operator-at-cursors (op motion count)` — выполнить operator+motion на всех курсорах
...
2. evm-operator-delete вызывается, показывает промпт "[EVM] d"
...
- Dependencies: evm-core, evil
- Interface:
  - `evm-run-normal (cmd)` — выполнить normal команду (\\z)
...
| `<s-mouse-1>` | `evm-add-cursor-at-click` | Добавить курсор по клику мыши |
...
| Key | Function | Описание |
|-----|----------|----------|
| `d` + motion | `evm-operator-delete` | Delete с motion (dw, de, diw, etc.) |
| `c` + motion | `evm-operator-change` | Change с motion (cw, ciw, etc.) |
| `y` + motion | `evm-operator-yank` | Yank с motion (yw, yiw, etc.) |
| `D` | `evm-delete-to-eol` | Delete до конца строки |
| `C` | `evm-change-to-eol` | Change до конца строки |
| `Y` | `evm-yank-line` | Yank строку |
...
| `J` | `evm-join-lines` | Объединить строки |
...
| Key | Function | Описание |
|-----|----------|----------|
| `<s-mouse-1>` | `evm-add-cursor-at-click` | Добавить курсор по клику |
